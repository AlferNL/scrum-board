...
export default function Board() {
  ...
  const allStoriesCollapsed = currentSprint?.stories?.length ? currentSprint.stories.every(s => collapsedStories.has(s.id)) : false;
  const toggleCollapseAll = () => {
    if (!currentSprint) return;
    if (allStoriesCollapsed) {
      setCollapsedStories(new Set());
    } else {
      setCollapsedStories(new Set(currentSprint.stories.map(s => s.id)));
    }
  };
            ...
            <div className="flex flex-col gap-1 px-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-400"
                  title={allStoriesCollapsed ? 'Alles uitklappen' : 'Alles inklappen'}
                >
                  {allStoriesCollapsed ? (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
